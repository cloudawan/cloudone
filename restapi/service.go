...
func registerWebServiceReplicationController() {
	...
	ws.Route(ws.GET("/{namespace}").To(getAllKubernetesService).
	...
	ws.Route(ws.DELETE("/{namespace}/{service}").To(deleteKubernetesService).
	...
	ws.Route(ws.POST("/json/{namespace}").To(postKubernetesServiceFromJson).
