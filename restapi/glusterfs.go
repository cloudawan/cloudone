...
func registerWebServiceGlusterfs() {
	...
	ws.Route(ws.POST("/clusters/{cluster}/volumes/").Filter(authorize).Filter(auditLog).To(postGlusterfsVolume).
		...
		Param(ws.PathParameter("cluster", "Cluster name").DataType("string")).
		Do(returns200, returns400, returns404, returns422, returns500).
		Reads(glusterfs.GlusterfsVolumeCreateParameter{}))
	...
	ws.Route(ws.PUT("/clusters/{cluster}/volumes/{volume}").Filter(authorize).Filter(auditLog).To(putGlusterfsVolume).
		Doc("Stop and delete the volume. Then create and start the glusterfs volume with the same parameters").
		Param(ws.PathParameter("cluster", "Cluster name").DataType("string")).
		Param(ws.PathParameter("volume", "Volume name").DataType("string")).
		Do(returns200, returns404, returns422, returns500))
}
...
func postGlusterfsVolume(request *restful.Request, response *restful.Response) {
	...
	glusterfsVolumeCreateParameter := &glusterfs.GlusterfsVolumeCreateParameter{}
	err := request.ReadEntity(&glusterfsVolumeCreateParameter)
	if err != nil {
		jsonMap := make(map[string]interface{})
		jsonMap["Error"] = "Read body failure"
		jsonMap["ErrorMessage"] = err.Error()
	...
	if err != nil {
		...
		jsonMap["ErrorMessage"] = err.Error()
		jsonMap["cluster"] = cluster
		jsonMap["glusterfsVolumeCreateParameter"] = glusterfsVolumeCreateParameter
		errorMessageByteSlice, _ := json.Marshal(jsonMap)
		log.Error(jsonMap)
		response.WriteErrorString(422, string(errorMessageByteSlice))
		return
	...
	err = glusterfsCluster.StartVolume(glusterfsVolumeCreateParameter.VolumeName)
	if err != nil {
		jsonMap := make(map[string]interface{})
...
func putGlusterfsVolume(request *restful.Request, response *restful.Response) {
	cluster := request.PathParameter("cluster")
	volume := request.PathParameter("volume")
	...
	err = glusterfsCluster.DeleteAndRecreateVolume(volume)
	if err != nil {
		jsonMap := make(map[string]interface{})
		jsonMap["Error"] = "Delete and recreate glusterfs volume failure"
		jsonMap["ErrorMessage"] = err.Error()
		jsonMap["cluster"] = cluster
		jsonMap["volume"] = volume
		errorMessageByteSlice, _ := json.Marshal(jsonMap)
